...
export default function ProjectForm({ initial, habitudes, onSubmit, onCancel, submitLabel }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <label className="block text-sm text-gray-300 mb-2">Nom du projet</label>
        <input
          type="text"
          value={nom}
      ...
      <div>
        <label className="block text-sm text-gray-300 mb-2">Description (optionnel)</label>
        <input
          type="text"
          value={description}
          ...
          {PROJECT_COLORS.map((c) => (
            <button
              ...
              className={`w-7 h-7 rounded-full border-2 transition-colors ${
                couleur === c ? 'border-white' : 'border-transparent'
              }`}
              style={{ background: c }}
              aria-label={c}
            />
      ...
      <div>
        <label className="block text-sm text-gray-300 mb-2">Habitude liée (optionnel)</label>
        <select
          value={habitudeId}
          onChange={(e) => setHabitudeId(e.target.value)}
          className="w-full bg-transparent border border-gray-700 rounded-md px-3 py-2 text-sm text-gray-300"
        >
      ...
      {habitudeId && (
        <div>
          <label className="block text-sm text-gray-300 mb-2">
            ...
            <span className="text-white font-bold">{tempsMinimum} min</span>
          </label>
          <input
            type="range"
            min={5}
